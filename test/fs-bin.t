...
# Compile test/fs.ys to binary once
try "gloat test/fs.ys -o test/fs"
is "$rc" 0 "fs.ys compiles to binary"
trap "rm -f test/fs" exit

# Test fs/mkdir
try "test/fs fs/mkdir $TMP/d1"
is "$rc" 0 "fs/mkdir runs"
try "test/fs fs-e $TMP/d1"
is "$got" "true" "fs/mkdir created directory"

# Test fs/mkdir-p
try "test/fs fs/mkdir-p $TMP/a/b/c"
is "$rc" 0 "fs/mkdir-p runs"
try "test/fs fs/dir? $TMP/a/b/c"
is "$got" "true" "fs/mkdir-p created nested dirs"

# Test fs/touch
try "test/fs fs/touch $TMP/f1.txt"
is "$rc" 0 "fs/touch runs"
try "test/fs fs/file? $TMP/f1.txt"
is "$got" "true" "fs/touch created file"

# Test fs/exists?
try "test/fs fs/exists? $TMP/f1.txt"
is "$got" "true" "fs/exists? returns true for existing file"
try "test/fs fs/exists? $TMP/nonexistent"
is "$got" "false" "fs/exists? returns false for missing file"

# Test fs/cp
echo "content" > "$TMP/orig.txt"
try "test/fs fs/cp $TMP/orig.txt $TMP/copy1.txt"
is "$rc" 0 "fs/cp runs"
try "test/fs fs/file? $TMP/copy1.txt"
is "$got" "true" "fs/cp created copy"

# Test fs/mv
echo "move me" > "$TMP/src1.txt"
try "test/fs fs/mv $TMP/src1.txt $TMP/dst1.txt"
is "$rc" 0 "fs/mv runs"
try "test/fs fs/file? $TMP/dst1.txt"
is "$got" "true" "fs/mv created destination"
try "test/fs fs/exists? $TMP/src1.txt"
is "$got" "false" "fs/mv removed source"

# Test fs/rm
echo "delete me" > "$TMP/del1.txt"
try "test/fs fs/rm $TMP/del1.txt"
is "$rc" 0 "fs/rm runs"
try "test/fs fs/exists? $TMP/del1.txt"
is "$got" "false" "fs/rm deleted file"

# Test fs/rm-r
mkdir -p "$TMP/rmr1/sub"
echo "content" > "$TMP/rmr1/sub/file.txt"
try "test/fs fs/rm-r $TMP/rmr1"
is "$rc" 0 "fs/rm-r runs"
try "test/fs fs/exists? $TMP/rmr1"
is "$got" "false" "fs/rm-r deleted directory tree"

# Test fs/rm-f
try "test/fs fs/rm-f $TMP/nonexistent1.txt"
is "$rc" 0 "fs/rm-f runs without error on missing file"

# Test fs/rmdir
mkdir "$TMP/empty1"
try "test/fs fs/rmdir $TMP/empty1"
is "$rc" 0 "fs/rmdir runs"
try "test/fs fs/exists? $TMP/empty1"

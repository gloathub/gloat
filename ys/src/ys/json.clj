...
(defn dump [x]
  ...
  (cond
    ...
    (sequential? x) (str "["
                         (str/join "," (map dump x))
                         "]")
    :else (throw (Exception. (str "Cannot encode " (type x) " to JSON")))))
      ...
      (let [c (nth s i)]
        (cond
          ...
          (= c \,) (let [new-pos (skip-whitespace s (inc i))
                         ...
                         new-pos (skip-whitespace s new-pos)]
                     (recur new-pos (conj values v)))
          :else (throw (Exception. (str "Unexpected character in array: " c))))))))
        ...
        (cond
          ...
          (= c \") (let [[k new-pos] (parse-string s i)
                     ...
                     (if (not= (nth s new-pos) \:)
                       (throw (Exception. "Expected : after object key"))
          ...
          :else (throw (Exception. (str "Unexpected character in object: " c))))))))
    ...
    (if (>= pos (count s))
      ...
      (let [c (nth s pos)]
        (cond
          ...
          [nil (+ pos 4)]
          :else (throw (Exception. (str "Unexpected character: " c))))))))
